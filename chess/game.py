...
class Board:
    def __init__(self, fen: str = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1") -> None:
        ...
        self.halfmoves = 0  # counts to 100 halfmoves
        self.moves = 0
    ...
    def get_legal_moves(self) -> list[Move]:
        ...
        # for square in self.position:
        #     if square.has_piece():
        #         for target_square in self.position:
        #             target_rank = index_to_coordinate(target_square.index)[1]
        #             if square.piece.type == PAWN and (target_rank == 0 or target_rank == 7):
        #                 for piece_type in LEGAL_PROMOTE_PIECES:
        #                     move = Move(square.index, target_square.index, Piece(piece_type, self.turn))
        #                     if move.is_valid(self):
        #                         moves.append(move)
        #             else:
        #                 move = Move(square.index, target_square.index)
        #                 if move.is_valid(self):
        #                     moves.append(move)
        for si in range(64):
            for ti in range(64):
    ...
    def is_check(self, colour: int) -> bool:
        """
...
#         mv = Move(algebraic_to_index(m[0]), algebraic_to_index(m[1]), Piece.from_str(m[2]))
#
#         if mv.is_valid(b):
